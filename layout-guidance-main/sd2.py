...
from my_model.sd2.unet_2d_condition2 import UNet2DConditionModel
# --------------------------------------------------------------------------------
        ...
        **kwargs,
        ...
        for i, t in enumerate(tqdm(noise_scheduler.timesteps)):
            ...
            while loss.item() / cfg.inference.loss_scale > cfg.inference.loss_threshold and \
                ...
                # print("迭代开始前已使用显存：", torch.cuda.memory_allocated(torch.cuda.current_device()) / (1024 ** 3))
                ...
                # --------------------------------------------------------------------------------
                del attn_map_integrated_up, attn_map_integrated_mid, attn_map_integrated_down
                ...
                latents = latents - grad_cond * noise_scheduler.sigmas[i] ** 2
                iteration += 1
                # 清理 CUDA 缓存以释放内存
                del grad_cond
                ...
                del noise_pred
                loss = loss.detach_()
                print("迭代结束后已使用显存：", torch.cuda.memory_allocated(torch.cuda.current_device()) / (1024 ** 3))
                # --------------------------------------------------------------------------------

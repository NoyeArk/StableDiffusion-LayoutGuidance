...
from diffusers.models.normalization import AdaLayerNormSingle, AdaLayerNormZero, AdaLayerNormContinuous
from diffusers.models.attention import GatedSelfAttentionDense
from diffusers.models.attention_processor import SpatialNorm, AttnProcessor2_0, \
        ...
        **kwargs,
        ...
        if len(args) > 0 or kwargs.get("scale", None) is not None:
            deprecation_message = ("The `scale` argument is deprecated and will be ignored. Please remove it, "
                                   "as passing it will raise an error in the future. `scale` should directly "
                                   "be passed while calling the underlying pipeline component i.e., via `cross_"
                                   "attention_kwargs`.")
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if cross_attention_dim is not None or double_self_attention:
            # 我们目前只使用 AdaLayerNormZero 进行自注意力，其中只有一个注意力块
